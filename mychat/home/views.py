...
def create_QA_model():
    ...
    qaoptimizer = adam_v2.Adam(lr=5e-5)
...
def es_search_body(value, key):
    ...
    """
    body = {
        "_source": ["context","question", "answer"],
        "query": {
            "match": {
                key: value
            }
...
def checkSimilarAnswer(answer):
    ...
    similar_texts = []
    for i in range(l):
        senttext2 = results['hits']['hits'][i]['_source']['context']
        print(senttext2)
        y = sim.get_score(senttext1, senttext2)
        print(y)
        if y >= 0.5:
            returnText = senttext2
